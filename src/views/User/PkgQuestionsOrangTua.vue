      ...
      <section v-if="dataPkg.data_guru != ''">
        ...
        <v-row>
          ...
          >
            ...
            >
              <v-card-title class="justify-center">{{ judulKomponen.judul }}</v-card-title>
              <v-divider></v-divider>
              <v-card-text>
                <v-form
                  ref="formQuestionPkg"
                ...
                >
                  <ol>
                    <section
                      v-for="(item, index) in pernyataan"
                      :key="index"
                    >
                      <li>{{ item.judul }}</li>
...
export default {
  ...
  methods: {
    ...
    getIdKomponen() {
      this.idKomponen = parseInt(this.$route.params.idQuestion)
    },
    ...
    async getCurrentQuestion() {
      try {
        ...
        const response = await axios.get(this.api_url + '/user/survey/data-komponen/' + this.idKomponen, config)

        this.pernyataan = response.data.data.pernyataan
        this.jawaban = response.data.data.jawaban
        this.judulKomponen = response.data.data.judul_komponen
    ...
    async nextQuestion() {
      ...
      if (!this.$refs.formQuestionPkg.validate()) {
        ...
        try {
          ...
          this.idKomponen = this.idKomponen + 1

          this.$router.push(`/orang-tua/pkg/create/${this.idKomponen}`)
    ...
    async finalStep() {
       ...
       if (!this.$refs.formQuestionPkg.validate()) {
        ...
        try {
          ...
          this.$router.push('/orang-tua/pkg/create/confirm')
